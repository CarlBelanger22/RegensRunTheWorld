...
describe('serializeBackup', () => {
  it('wraps players in a versioned envelope', () => {
    const players = [samplePlayer()]
    const backup = serializeBackup(players, '2026-09-06T00:00:00.000Z')
...
describe('parseBackup', () => {
  it('round-trips a serialized envelope', () => {
    ...
    const json = JSON.stringify(serializeBackup(players, '2026-09-06T00:00:00.000Z'))
    const result = parseBackup(json)
    expect(result).toEqual({ ok: true, players })
  })

  it('accepts a bare players array', () => {
    const players = [samplePlayer()]
